...
from monitor_bot.db_models import AgendaItem, MonitoredSource, SearchQuery, SearchResult, SearchRun, SourceCategory, SourceType
...
async def _create_source_if_new(
    ...
    if await source_svc.source_url_exists(db, url, owner_user_id=owner_user_id):
        return False
    await source_svc.create_source(
        db,
        owner_user_id,
        SourceCreate(name=name, url=url, category=category, source_type=source_type),
    )
    return True


async def _backfill_legacy_ownership(db: AsyncSession, owner_user_id: int) -> None:
    """Assign old single-tenant rows to the bootstrap admin account."""
    for model in (MonitoredSource, SearchQuery, SearchRun, SearchResult, AgendaItem):
        await db.execute(
            update(model)
...
async def seed_defaults(db: AsyncSession, settings: Settings) -> None:
    ...
    await _backfill_legacy_ownership(db, admin_user.id)
    existing_sources = await source_svc.count_sources(db, owner_user_id=admin_user.id)
    existing_queries = await query_svc.count_queries(db, owner_user_id=admin_user.id)
    ...
    for name, url, stype in BUILTIN_BANDI_SOURCES:
        if await _create_source_if_new(db, admin_user.id, name, url, SourceCategory.BANDI, stype):
            created_sources += 1
    ...
    for text in settings.web_search_queries:
        if not await query_svc.query_text_exists(db, text, owner_user_id=admin_user.id):
